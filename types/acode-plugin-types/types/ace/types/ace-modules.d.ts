...
declare module "ace/layer/font_metrics" {
	...
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type FontMetricsEvents = Ace.FontMetricsEvents;
	}
	export interface FontMetrics
		extends Ace.EventEmitter<Ace.FontMetricsEvents> {}
}
declare module "ace/apply_delta" {
...
declare module "ace/document" {
	...
	export type Delta = Ace.Delta;
	export type Point = Ace.Point;
	export type IRange = Ace.IRange;
	export type NewLineMode = Ace.NewLineMode;
	import { Anchor } from "ace/anchor";
	...
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type DocumentEvents = Ace.DocumentEvents;
	}
...
declare module "ace/anchor" {
	...
	export type Document = import("ace/document").Document;
	namespace Ace {
		type EventEmitter<
			T extends {
				[K in keyof T]: (...args: any[]) => any;
		...
		> = Ace.EventEmitter<T>;
		type AnchorEvents = Ace.AnchorEvents;
		type Document = Ace.Document;
	}
...
declare module "ace/config" {
	const _exports: {
		 ...
		 * @returns {Ace.ConfigOptions[K]} - The value of the config option.
		 */
		get: <K extends keyof Ace.ConfigOptions>(
			key: K,
		) => Ace.ConfigOptions[K];
		set: <K extends keyof Ace.ConfigOptions>(
			key: K,
			value: Ace.ConfigOptions[K],
		) => void;
		all: () => Ace.ConfigOptions;
		/**
		 * module loading
		 */
...
declare module "ace/layer/lines" {
	export type EditSession = import("ace/edit_session").EditSession;
	export type LayerConfig = Ace.LayerConfig;
	export class Lines {
...
declare module "ace/layer/gutter" {
	export class Gutter {
		...
		getRegion(point: { x: number }): "markers" | "foldWidgets";
	}
	export type EditSession = import("ace/edit_session").EditSession;
	export type LayerConfig = Ace.LayerConfig;
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type GutterEvents = Ace.GutterEvents;
	}
	export interface Gutter extends Ace.EventEmitter<Ace.GutterEvents> {}
}
declare module "ace/layer/marker" {
	export type EditSession = import("ace/edit_session").EditSession;
	export type LayerConfig = Ace.LayerConfig;
	export class Marker {
		constructor(parentEl: HTMLElement);
		...
		setSession(session: EditSession): void;
		session: import("ace/edit_session").EditSession;
		setMarkers(markers: { [x: number]: Ace.MarkerLike }): void;
		markers: {
...
declare module "ace/layer/text_util" {
	export function isTextToken(tokenType: any): boolean;
}
declare module "ace/layer/text" {
	export class Text {
		constructor(parentEl: HTMLElement);
		dom: typeof dom;
		element: HTMLDivElement;
	...
	export type LayerConfig = Ace.LayerConfig;
	export type EditSession = import("ace/edit_session").EditSession;
	import dom = require("ace/lib/dom");
	namespace Ace {
		type EventEmitter<
			T extends {
				[K in keyof T]: (...args: any[]) => any;
		...
		> = Ace.EventEmitter<T>;
		type TextEvents = Ace.TextEvents;
		type LayerConfig = Ace.LayerConfig;
	}
	export interface Text extends Ace.EventEmitter<Ace.TextEvents> {
		config: Ace.LayerConfig;
	}
}
declare module "ace/layer/cursor" {
	export class Cursor {
		constructor(parentEl: HTMLElement);
		element: HTMLDivElement;
		isVisible: boolean;
...
declare module "ace/scrollbar" {
	...
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type ScrollbarEvents = Ace.ScrollbarEvents;
	}
...
declare module "ace/scrollbar_custom" {
	const VScrollBar_base: typeof ScrollBar;
	/**
	 * Represents a vertical scroll bar.
	 * @class VScrollBar
...
declare module "ace/css/editor-css" {
	const _exports: string;
	export = _exports;
}
declare module "ace/layer/decorators" {
...
declare module "ace/virtual_renderer" {
	/**
	...
	export class VirtualRenderer {
		...
		scrollBarV: Ace.VScrollbar;
		scrollBarH: Ace.HScrollbar;
		scrollTop: number;
		scrollLeft: number;
		cursorPos: {
			row: number;
		 ...
		 *
		 **/
		setAnnotations(annotations: Ace.Annotation[]): void;
		/**
		 *
	...
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type VirtualRendererEvents = Ace.VirtualRendererEvents;
		type OptionsProvider<T> = Ace.OptionsProvider<T>;
		type VirtualRendererOptions = Ace.VirtualRendererOptions;
		type EditSession = Ace.EditSession;
	}
	export interface VirtualRenderer
		extends Ace.EventEmitter<Ace.VirtualRendererEvents>,
			Ace.OptionsProvider<Ace.VirtualRendererOptions> {
		textarea: HTMLTextAreaElement;
		enableKeyboardAccessibility?: boolean;
		showInvisibles?: boolean;
		theme?: any;
...
declare module "ace/selection" {
	export class Selection {
		...
		session: EditSession;
		doc: import("ace/document").Document;
		cursor: Anchor;
		lead: Anchor;
		anchor: Anchor;
		/**
		 ...
		 * @param {Ace.IRange} range The range of text to select
		 * @param {Boolean} [reverse] Indicates if the range should go backwards (`true`) or not
		 **/
		setRange(range: Ace.IRange, reverse?: boolean): void;
		/**
	...
	export type EditSession = import("ace/edit_session").EditSession;
	export type Anchor = import("ace/anchor").Anchor;
	export type Point = Ace.Point;
	import { Range } from "ace/range";
	namespace Ace {
		type EventEmitter<
		...
		> = Ace.EventEmitter<T>;
		type MultiSelectionEvents = Ace.MultiSelectionEvents;
		type MultiSelectProperties = Ace.MultiSelectProperties;
	}
	export interface Selection
		extends Ace.EventEmitter<Ace.MultiSelectionEvents>,
			Ace.MultiSelectProperties {}
}
declare module "ace/clipboard" {
...
declare module "ace/mouse/default_handlers" {
	export type MouseHandler = import("ace/mouse/mouse_handler").MouseHandler;
	export type MouseEvent = import("ace/mouse/mouse_event").MouseEvent;
	export class DefaultHandlers {
		constructor(mouseHandler: MouseHandler);
		onMouseDown(
			this: import("ace/mouse/mouse_handler").MouseHandler,
			ev: MouseEvent,
		): void;
		mousedownEvent: import("ace/mouse/mouse_event").MouseEvent;
		startSelect(
			this: import("ace/mouse/mouse_handler").MouseHandler,
			pos?: Ace.Position,
			waitForClickSelection?: boolean,
		): void;
		select(this: import("ace/mouse/mouse_handler").MouseHandler): void;
		extendSelectionBy(
			this: import("ace/mouse/mouse_handler").MouseHandler,
		...
		focusWait(this: import("ace/mouse/mouse_handler").MouseHandler): void;
		onDoubleClick(
			this: import("ace/mouse/mouse_handler").MouseHandler,
			ev: MouseEvent,
		): void;
		...
		selectEnd: (this: import("ace/mouse/mouse_handler").MouseHandler) => void;
		selectAllEnd: (
			this: import("ace/mouse/mouse_handler").MouseHandler,
		) => void;
		selectByWordsEnd: (
...
declare module "ace/tooltip" {
	export class HoverTooltip extends Tooltip {
		...
		lastT: number;
		idleTime: number;
		lastEvent: import("ace/mouse/mouse_event").MouseEvent;
		waitForHover(): void;
...
declare module "ace/mouse/default_gutter_handler" {
	export function GutterHandler(
		this: import("ace/mouse/mouse_handler").MouseHandler,
...
declare module "ace/mouse/dragdrop_handler" {
	export type MouseHandler = import("ace/mouse/mouse_handler").MouseHandler;
...
declare module "ace/mouse/touch_handler" {
	export function addTouchListeners(el: any, editor: any): void;
}
declare module "ace/mouse/mouse_handler" {
	...
	export type Editor = import("ace/editor").Editor;
	namespace Ace {
		type Range = Ace.Range;
		type MouseEvent = Ace.MouseEvent;
		type Point = Ace.Point;
	}
...
declare module "ace/mouse/fold_handler" {
...
declare module "ace/keyboard/keybinding" {
	export type Editor = import("ace/editor").Editor;
	export type KeyboardHandler = Ace.KeyboardHandler;
...
declare module "ace/search" {
	export type EditSession = import("ace/edit_session").EditSession;
	export type SearchOptions = Ace.SearchOptions;
	/**
...
declare module "ace/keyboard/hash_handler" {
	export type Command = Ace.Command;
	export type CommandLike = Ace.CommandLike;
	...
	export class MultiHashHandler {
		constructor(
			config?: Record<string, CommandLike> | Command[],
			platform?: Platform,
		);
		platform: Platform;
...
declare module "ace/commands/command_manager" {
	const CommandManager_base: typeof MultiHashHandler;
	export class CommandManager extends MultiHashHandler {
		/**
		 * new CommandManager(platform, commands)
		 * @param {import("ace/keyboard/hash_handler").Platform} platform Identifier for the platform; must be either `"mac"` or `"win"`
		 * @param {any[]} commands A list of commands
		 **/
		constructor(
			platform: import("ace/keyboard/hash_handler").Platform,
			commands: any[],
		);
		byName: Record<string, Ace.Command>;
		exec(
			command: string | string[] | Ace.Command,
			editor: Editor,
			args: any,
		): boolean;
		canExecute(
	...
	export type Editor = import("ace/editor").Editor;
	import { MultiHashHandler } from "ace/keyboard/hash_handler";
	namespace Ace {
		type EventEmitter<
			T extends {
				[K in keyof T]: (...args: any[]) => any;
		...
		> = Ace.EventEmitter<T>;
		type CommandManagerEvents = Ace.CommandManagerEvents;
	}
	export interface CommandManager
		extends Ace.EventEmitter<Ace.CommandManagerEvents> {}
}
declare module "ace/commands/default_commands" {
	export const commands: Ace.Command[];
}
declare module "ace/token_iterator" {
	export type EditSession = import("ace/edit_session").EditSession;
	/**
	...
	export class TokenIterator {
		 ...
		 **/
		stepBackward(): Ace.Token | null;
		/**
		 * Moves iterator position to the start of next token.
		 **/
		stepForward(): Ace.Token | null;
		/**
		 *
		 * Returns current token.
		 **/
		getCurrentToken(): Ace.Token;
		/**
		 *
...
declare module "ace/keyboard/gutter_handler" {
	...
	import { GutterTooltip } from "ace/mouse/default_gutter_handler";
}
declare module "ace/editor" {
	...
	export class Editor {
		 ...
		 * @param {Partial<Ace.EditorOptions>} [options] The default options
		 **/
		constructor(
		 ...
		 **/
		setKeyboardHandler(
			keyboardHandler: string | Ace.KeyboardHandler | null,
			cb?: () => void,
		): void;
		/**
		 ...
		 **/
		setSession(session?: EditSession): void;
		selection: import("ace/selection").Selection;
		/**
		 ...
		 **/
		setTheme(theme: string | Ace.Theme, cb?: () => void): void;
		/**
		 ...
		 **/
		getSelectionStyle(): Ace.EditorOptions["selectionStyle"];
		/**
	...
	export type VirtualRenderer = import("ace/virtual_renderer").VirtualRenderer;
	export type Selection = import("ace/selection").Selection;
	export type Point = Ace.Point;
	export type SearchOptions = Ace.SearchOptions;
	...
	var $uid: number;
	namespace Ace {
		type EditorMultiSelectProperties =
			Ace.EditorMultiSelectProperties;
		type OptionsProvider<T> = Ace.OptionsProvider<T>;
		type EditorOptions = Ace.EditorOptions;
		type EventEmitter<
			T extends {
				[K in keyof T]: (...args: any[]) => any;
			},
		> = Ace.EventEmitter<T>;
		type EditorEvents = Ace.EditorEvents;
		type CodeLenseEditorExtension = Ace.CodeLenseEditorExtension;
		type ElasticTabstopsEditorExtension =
			Ace.ElasticTabstopsEditorExtension;
		type TextareaEditorExtension = Ace.TextareaEditorExtension;
		type PromptEditorExtension = Ace.PromptEditorExtension;
		type OptionsEditorExtension = Ace.OptionsEditorExtension;
		type EditSession = Ace.EditSession;
		type LineWidgets = Ace.LineWidgets;
		type Autocomplete = Ace.Autocomplete;
		type InlineAutocomplete = Ace.InlineAutocomplete;
		type Completer = Ace.Completer;
		type SearchBox = Ace.SearchBox;
	}
	export interface Editor
		extends Ace.EditorMultiSelectProperties,
			Ace.OptionsProvider<Ace.EditorOptions>,
			Ace.EventEmitter<Ace.EditorEvents>,
			Ace.CodeLenseEditorExtension,
			Ace.ElasticTabstopsEditorExtension,
...
declare module "ace/undomanager" {
	...
	export class UndoManager {
		...
		getRevision(): number;
		getDeltas(from: number, to?: number): Ace.Delta[];
...
declare module "ace/autocomplete/popup" {
	...
	namespace Ace {
		type AcePopupWithEditor = Ace.AcePopupWithEditor;
		type Completion = Ace.Completion;
		type Point = Ace.Point;
		type AcePopupNavigation = Ace.AcePopupNavigation;
	}
...
declare module "ace/range_list" {
	...
	export class RangeList {
		...
		detach(): void;
		comparePoints: (
			p1: import("ace/range").Point,
			p2: import("ace/range").Point,
		) => number;
	}
	import { Range } from "ace/range";
}
declare module "ace/snippets" {
	...
	import { Tokenizer } from "ace/tokenizer";
	namespace Ace {
		type EventEmitter<
			T extends {
				[K in keyof T]: (...args: any[]) => any;
...
declare module "ace/autocomplete/inline_screenreader" {
	/**
	...
	export class AceInlineScreenReader {
		 ...
		 */
		constructor(editor: import("ace/editor").Editor);
		editor: import("ace/editor").Editor;
		...
		popup: import("ace/autocomplete/popup").AcePopup;
		destroy(): void;
		/**
...
declare module "ace/autocomplete/inline" {
	export type Editor = import("ace/editor").Editor;
	/**
	...
	import { AceInlineScreenReader } from "ace/autocomplete/inline_screenreader";
}
declare module "ace/autocomplete/util" {
	export function parForEach(array: any, fn: any, callback: any): void;
	export function retrievePrecedingIdentifier(
		text: any,
		pos: any,
		regex: any,
	): string;
	export function retrieveFollowingIdentifier(
		text: any,
		pos: any,
		regex: any,
	): any[];
...
declare module "ace/autocomplete" {
	...
	export class Autocomplete {
		...
		goTo(where: any): void;
		insertMatch(data: Completion, options?: undefined): boolean | undefined;
		/**
	...
	export class CompletionProvider {
		...
		gatherCompletions(
			editor: Editor,
			callback: Ace.CompletionCallbackFunction,
		): boolean;
		completers: Ace.Completer[];
		/**
	...
	export type CompletionOptions = Ace.CompletionOptions;
	export type Position = Ace.Position;
	...
	namespace Ace {
		type AcePopup = Ace.AcePopup;
		type FilteredList = Ace.FilteredList;
	}
...
declare module "ace/autocomplete/text_completer" {
...
declare module "ace/line_widgets" {
	...
	export type Editor = import("ace/editor").Editor;
	export type VirtualRenderer = import("ace/virtual_renderer").VirtualRenderer;
	export type LineWidget = Ace.LineWidget;
	namespace Ace {
		type LineWidget = Ace.LineWidget;
...
declare module "ace/search_highlight" {
	export type Marker = import("ace/layer/marker").Marker;
	export type EditSession = import("ace/edit_session").EditSession;
	export class SearchHighlight {
		...
		update(
			...
			markerLayer: Marker,
			session: EditSession,
			config: Partial<Ace.LayerConfig>,
		): void;
		MAX_RANGES: number;
	}
}
declare module "ace/occur" {
	export type Editor = import("ace/editor").Editor;
	export type Point = Ace.Point;
	export type SearchOptions = Ace.SearchOptions;
	/**
	...
	import { Search } from "ace/search";
	import { EditSession } from "ace/edit_session";
}
declare module "ace/marker_group" {
	export type EditSession = import("ace/edit_session").EditSession;
	export type MarkerGroupItem = {
		range: import("ace/range").Range;
		className: string;
	};
	export type LayerConfig = Ace.LayerConfig;
	export type Marker = import("ace/layer/marker").Marker;
...
declare module "ace/edit_session/fold" {
	export class Fold extends RangeList {
		...
		foldLine: import("ace/edit_session/fold_line").FoldLine;
		placeholder: any;
		range: import("ace/range").Range;
		start: Ace.Point;
		end: Ace.Point;
	...
	export type FoldLine = import("ace/edit_session/fold_line").FoldLine;
	export type Range = import("ace/range").Range;
	export type Point = Ace.Point;
	export type IRange = Ace.IRange;
	import { RangeList } from "ace/range_list";
	export interface Fold {
		collapseChildren?: number;
	}
}
declare module "ace/edit_session/fold_line" {
	export type Fold = import("ace/edit_session/fold").Fold;
	export class FoldLine {
		...
		toString(): string;
		idxToPosition(idx: number): Ace.Point;
	}
	import { Range } from "ace/range";
}
declare module "ace/bidihandler" {
	export type EditSession = import("ace/edit_session").EditSession;
	/**
	...
	import bidiUtil = require("ace/lib/bidiutil");
}
declare module "ace/background_tokenizer" {
	...
	export class BackgroundTokenizer {
		...
		currentLine: number;
		tokenizer: import("ace/tokenizer").Tokenizer;
		/**
	...
	export type Document = import("ace/document").Document;
	export type EditSession = import("ace/edit_session").EditSession;
	export type Tokenizer = import("ace/tokenizer").Tokenizer;
	namespace Ace {
		type EventEmitter<
			T extends {
				[K in keyof T]: (...args: any[]) => any;
		...
		> = Ace.EventEmitter<T>;
		type BackgroundTokenizerEvents =
			Ace.BackgroundTokenizerEvents;
	}
	export interface BackgroundTokenizer
		extends Ace.EventEmitter<Ace.BackgroundTokenizerEvents> {}
}
declare module "ace/edit_session/folding" {
	export type IFolding = import("ace/edit_session").EditSession &
		Ace.Folding;
	export type Delta = Ace.Delta;
	...
	export class Folding {
		 ...
		 **/
		getFoldsInRange: (
			range: Ace.Range | Ace.Delta,
		) => Ace.Fold[];
		getFoldsInRangeList: (
			ranges: Ace.Range[] | Ace.Range,
		) => Ace.Fold[];
		/**
...
declare module "ace/edit_session/bracket_match" {
	export type EditSession = import("ace/edit_session").EditSession;
	export type Point = import("ace/edit_session").Point;
	export function BracketMatch(): void;
	export class BracketMatch {
		findMatchingBracket: (
			this: import("ace/edit_session").EditSession,
			position: Point,
			chr?: string,
		) => Ace.Point;
		getBracketRange: (
...
declare module "ace/edit_session" {
	/**
	...
	export class EditSession {
		 ...
		 *
		 **/
		getTokenAt(row: number, column: number): Ace.Token;
		/**
		 ...
		 * @param {Ace.MarkerRenderer | "fullLine" | "screenLine" | "text" | "line"} [type] Identify the renderer type of the marker. If string provided, corresponding built-in renderer is used. Supported string types are "fullLine", "screenLine", "text" or "line". If a Function is provided, that Function is used as renderer.
		 ...
		 **/
		setNewLineMode(newLineMode: Ace.NewLineMode): void;
		/**
		 *
		 ...
		 **/
		getValue: () => string;
		lineWidgets: null | Ace.LineWidget[];
	...
	export type FontMetrics = import("ace/layer/font_metrics").FontMetrics;
	export type FoldLine = import("ace/edit_session/fold_line").FoldLine;
	export type Point = Ace.Point;
	export type Delta = Ace.Delta;
	export type IRange = Ace.IRange;
	export type SyntaxMode = Ace.SyntaxMode;
	export type LineWidget = Ace.LineWidget;
	...
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type EditSessionEvents = Ace.EditSessionEvents;
		type OptionsProvider<T> = Ace.OptionsProvider<T>;
		type EditSessionOptions = Ace.EditSessionOptions;
		type Folding = Ace.Folding;
		type BracketMatch = Ace.BracketMatch;
		type Document = Ace.Document;
		type Point = Ace.Point;
		type Occur = Ace.Occur;
		type Operation = Ace.Operation;
	}
	export interface EditSession
		extends Ace.EventEmitter<Ace.EditSessionEvents>,
...
declare module "ace/placeholder" {
	export class PlaceHolder {
		...
		updateAnchors(delta: Ace.Delta): void;
		updateMarkers(): void;
		/**
		 * PlaceHolder.detach()
		 *
	...
	namespace Ace {
		...
		> = Ace.EventEmitter<T>;
		type PlaceHolderEvents = Ace.PlaceHolderEvents;
	}
	export interface PlaceHolder
		extends Ace.EventEmitter<Ace.PlaceHolderEvents> {}
}
declare module "ace/mouse/multi_select_handler" {
	export function onMouseDown(e: any): any;
}
declare module "ace/commands/multi_select_commands" {
	export const defaultCommands: Ace.Command[];
	export const multiSelectCommands: Ace.Command[];
	export const keyboardHandler: HashHandler;
	import { HashHandler } from "ace/keyboard/hash_handler";
}
declare module "ace/multi_select" {
	export const commands: Ace.Command[];
	export const onSessionChange: (e: any) => void;
	export type Anchor = import("ace/anchor").Anchor;
	export type Point = Ace.Point;
	export type ScreenCoordinates = Ace.ScreenCoordinates;
	export function MultiSelect(editor: Editor): void;
}
declare module "ace/commands/occur_commands" {
...
declare module "ace/commands/incremental_search_commands" {
...
declare module "ace/incremental_search" {
	/**
	...
	import { Search } from "ace/search";
	import iSearchCommandModule = require("ace/commands/incremental_search_commands");
	import { Range } from "ace/range";
}
declare module "ace/split" {
	export type ISplit = Ace.EventEmitter<any> & {
		[key: string]: any;
	};
	export var Split: any;
}
declare module "ace/tokenizer_dev" {
	...
	import { Tokenizer as BaseTokenizer } from "ace/tokenizer";
}
declare module "ace/unicode" {
	export const wordChars: any;
}
declare module "ace/keyboard/textarea" {
